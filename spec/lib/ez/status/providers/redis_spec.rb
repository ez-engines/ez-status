...
RSpec.describe Ez::Status::Providers::Redis do
  describe 'check' do
    ...
    context 'successes' do
      # before(:each) do
      #   allow(Redis.cache).to receive(:write).with(described_class.to_s, time_now).and_return(true)
    ...
    context 'failure' do
      ...
      it 'should return answer' do
        expect { described_class.new.check }.to raise_error(Ez::Status::Providers::Redis)
      end
    end
  end
end
